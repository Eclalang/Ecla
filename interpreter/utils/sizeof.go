...
func sizeOf(v reflect.Value, cache map[uintptr]bool) int {
	...
	case reflect.Ptr:
		// return Ptr size if this node has been visited
		if cache[v.Pointer()] {
			return int(v.Type().Size())
		}
		cache[v.Pointer()] = true
	...
	case reflect.Bool, reflect.Uint8, reflect.Uint16, reflect.Uint32, reflect.Uint64, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64, reflect.Int, reflect.Uint, reflect.Chan, reflect.Uintptr, reflect.Float32, reflect.Float64, reflect.Complex64, reflect.Complex128, reflect.Func:
		return int(v.Type().Size())
	case reflect.Map:
		// return 0 if this node has been visited
		if cache[v.Pointer()] {
			return 0
		}
		cache[v.Pointer()] = true
